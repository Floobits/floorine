...
Object.keys(log_levels).forEach(function (key) {
  ...
  log_levels_to_string[value] = util.format("  %s", key.toUpperCase()).slice(-5);
});
...
var log = function (level_name, args, caller) {
  ...
  if (level <= log_level) {
    ...
    log_levels_to_func[level_name](util.format("%s%s \x1b[0m\x1b[40;1;30m%s\x1b[0m %s", log_levels_to_color[level], log_levels_to_string[level], now, log_str));
